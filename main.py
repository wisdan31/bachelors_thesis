...
from policies import BFSPolicy, GreedyBestFirstPolicy, DFSPolicy, RandomPolicy, LRTAStarPolicy, FSA_AStarPolicy, DStarLitePolicy
...
def run_menu():
    ...
    agent_options = ["Random", "DFS", "BFS", "Greedy Best-First", "LRTA*", "FSA-A*", "D* Lite"]
    agent_colors = {
        "Random": (180, 100, 50),
        "DFS":    (50, 140, 180),
        "BFS":    (50, 180, 100),
        "Greedy Best-First": (200, 180, 50),
        "LRTA*":  (150, 50, 200),
        "FSA-A*": (50, 200, 150),
        "D* Lite":(200, 50, 150),
    }
    ...
    for name in agent_options:
        agent_buttons.append(Button(20, abtn_y, 145 if name not in ["Greedy Best-First", "D* Lite"] else 300, 36, name, font_btn))
        if name not in ["Greedy Best-First", "D* Lite"] and agent_options.index(name) % 2 == 0:
            agent_buttons[-1].rect.x = 20
        elif name not in ["Greedy Best-First", "D* Lite"]:
            agent_buttons[-1].rect.x = 175
            abtn_y += 44
        else:
            agent_buttons[-1].rect.x = 20
            abtn_y += 44
...
def run_simulation(grid, agent_name, maze_size):
    ...
    policy_map = {
        ...
        "Greedy Best-First": GreedyBestFirstPolicy(goal_pos=goal),
        "LRTA*":             LRTAStarPolicy(goal_pos=goal),
        "FSA-A*":            FSA_AStarPolicy(goal_pos=goal, grid_size=size),
        "D* Lite":           DStarLitePolicy(goal_pos=goal, grid_size=size),
    }
